...
- **A PHPStan rule set enforcing the DSL, bundled from day one.** Four rules, one directory per abstraction type: `Rule/Class` seals concrete builders as final, `Rule/Method` restricts the public surface to the DSL and requires every modifier to be a static-returning one-liner through `mutate()`, `Rule/Property` keeps builder state private, per-instance and never readonly - readonly state would make `mutate()` throw at runtime. One `includes:` line for `extension.neon` turns the naming table into analysis errors; PHPStan itself stays an optional `suggest`.
